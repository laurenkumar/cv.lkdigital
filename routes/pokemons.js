            ...
            Competence.find({}).then(competence => {
                Formation.find({}).then(formation => {
                    Experience.find({}).then(experiences => {
                        Admin.find({ 'name': 'Dorian Laurenceau' }).then(admins => {
                            Infos.find({ 'email': 'lauren_d@outlook.fr' }).then(infos => {
                                Pokemon.find({}).then(pokemons => {
                                    if (req.session.passport) {
                                        res.render('pokemons/lui.html', { admins: admins, loisirs: loisirs, experiences: experiences, titrecompetence:titrecompetence, formation:formation, competence:competence, pokemons: pokemons,  infos: infos, user: req.session.passport.user });
                                    } else {
                                       res.render('pokemons/lui.html', { admins: admins, loisirs: loisirs, experiences: experiences, titrecompetence:titrecompetence, formation:formation, competence:competence, pokemons: pokemons,  infos: infos});
                                    }
                                });
                            });
                        });
